...
dt=1e-2;
dts = [0.05, 0.02, 0.01, 0.003, 0.001];
std_energy = zeros(1, length(dts));
for i = 1:length(dts)
    dt = dts(i);  % Current time step
    ...
    std_energy(i) = std(Ek + Ep + sum(Ewp, 2));  % Standard deviation of total energy
...
%%% Plot of Standard Deviation of Energy versus time step

clf;
loglog(dts, std_energy, '-o');
xlabel('Log(dt)');
ylabel('Log(Standard Deviation of Energy)');
title('Energy Conservation vs Time Step');
grid on;

x = polyfit(log(dts), log(std_energy), 1);

slope = x(1) % Slope is around 1.9 - 2, which means that 
