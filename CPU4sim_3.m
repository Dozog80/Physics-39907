...
if(~exist('prog','var') || isempty(prog))
  ...
  % code (Multiply A by B, result in 12)

  %%% Add A to itself B times. Add A -> decrement B by 1 -> repeat until
  %%% zero, or if negative then B was initially 0 so result is 0. Final
  %%% result is stored in ADDR 12


  prog{1}='LDB 14'; % Putting first number into register B
  prog{2}='LDD 15'; % Second number is in register D (to decrement each loop)
  prog{3}='LDC 13'; % Store a 1 in C, used to decrement second number (in D)
  prog{4}='ADD A B';  % Adding first number to itself, intermediate values stored in A, eventually stored in ADDR 12
  prog{5}='SUB D C';  % Decrementing D by 1 each loop
  prog{6}='JPN 11';   %% Halt (since subtraction is negative, second number was initially 0)
  prog{7}='JPZ 8';   %% Multiplication is over, time to store result and halt
  prog{8}='JMP 3';   % Loop over the additions
  prog{9}='STA 12'; % Store final result in ADDR 12
  prog{10}='JMP 11'; % Halt
  prog{11}='STD 12'; % If result of subtraction is negative, we are multiplying by 0 so result is 0.
  prog{12}='HLT';          

  % data
  prog{14} = 1; % 1 stored in ADDR 13 to decrement B each addition
  prog{15}= 10;          
  prog{16}= 8; % ADDR 15            
