...
if(~exist('prog','var') || isempty(prog))
  ...
  % code (Find Max of A and B) 

  %%% Subtract A and B (A - B), if negative B is larger -> store B, if positive, A is larger -> store A. 

  prog{1}='LDA 14';  
  prog{2}='LDB 15';   
  prog{3}='SUB A B';  % Stored in A
  prog{4}='JPN 07';   %% JPN 8th if A < B
  prog{5}='LDA 14';   % Load A from 14 (since A has result of sub)
  prog{6}='STA 13';   % Stores A in 13 if A > B %
  prog{7}='JMP 09';   % Halt after storing A
  prog{8}='LDA 15';   %% Load B into A
  prog{9}='STA 13';   %% Stores B in 13 if A < B %%
  prog{10}='HLT';          
